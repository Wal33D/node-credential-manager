...
export const addServiceFunction = async ({ dbConnection, collectionName, serviceName, }:
  { dbConnection: Db; collectionName: string; serviceName: string; }): Promise<{ status: boolean; message: string }> => {
  let status = false;
  let message = '';
