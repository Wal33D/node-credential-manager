        ...
        try {
            ...
            const decryptedSecrets = secretsArray.map(secret => ({
                ...
                versions: secret.versions.map(version => {
                    if (version.iv) {
                        return { ...version, value: decrypt({ hash: { iv: version.iv, content: version.value } }) };
                    }
                    return version;
                })
            }));
    
            return { status: true, message: "All secrets retrieved successfully.", secrets: decryptedSecrets };
        } catch (error) {
